        ...
        [node {:title "Connect Shapes"
         ...
         [:label {:class "flex gap-2 items-center mb-4 text-xl"}
          ...
          [:input {:type "number"
                   :value @max-shapes
                   :on-change (fn [e]
                                (let [new-max (-> e .-target .-value int)]
                                  (reset! max-shapes new-max)
                                    ...
                                    (re-frame/dispatch [::events/compute-with-loading id
                                                        (fn [_db]
                                                          (let [connections (if (every? #(contains? % :name) @parent-data)
                                                                              ...
                                                                              (search/memoize-connect (map :notes @parent-data) :chord :max-shapes new-max))]
                                                            {:connections connections}))]))))
                   :class "p-1 rounded-md border border-gray-400 nodrag text-black"
                   :size 2}]]
         (if (> (count @parent-data) 1)
           (do
             ;; Trigger computation with loading state for connection analysis
             (when (not (:connections @data))
               (re-frame/dispatch [::events/compute-with-loading id
                                   (fn [_db]
                                     (let [connections (if (every? #(contains? % :name) @parent-data)
                                                         (search/memoize-connect-shapes @parent-data :chord)
                                       ...
                                       {:connections connections}))]))
             (when-let [connections (:connections @data)]
